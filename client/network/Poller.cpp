...
void Poller::addFileDescriptor(int fileDescriptor, int events) {
  ...
  pollFileDescriptor.events = static_cast<int16_t>(events);
  pollFileDescriptor.revents = 0;
  _fds.push_back(pollFileDescriptor);
}

void Poller::updateWatchedEvents(int fileDescriptor, int events) {
  auto iter = std::ranges::find_if(
      _fds, [fileDescriptor](const struct pollfd& entry) {
...
std::vector<PollEvent> Poller::waitForEvents(int timeout) {
  const int ready =
      sys::Posix::poll(_fds.data(), static_cast<nfds_t>(_fds.size()), timeout);
